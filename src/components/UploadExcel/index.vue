<template>
  <input ref="refInput" class="excel-upload-input" type="file" accept=".xlsx, .xls" @change="handleClick" />
...
const beforeUpload = (rawFile) => {
  ...
  ElMessage.warning(`Please do not upload files larger than ${props.size}m in size.`)
  return false
}
...
const handleDrop = (e) => {
  ...
  if (files.length !== 1) return ElMessage.error('Only support uploading one file!')
  const rawFile = files[0] // only use files[0]

  if (!isExcel(rawFile)) return ElMessage.error('Only supports upload .xlsx, .xls, .csv suffix files')
  upload(rawFile)
}
